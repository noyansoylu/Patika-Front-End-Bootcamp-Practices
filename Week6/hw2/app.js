...
function displayMenu(menuItems) {
  const menuHTML = menuItems.map((item) => `
    <article class="menu-items col-12 col-md-6">
...
function buttonSetup() {
  const buttonHTML = categories.map((category) => `
    <button class="btn btn-outline-dark btn-item" data-category="${category}">
      ${category}
    </button>
  `).join("");
  buttons.innerHTML = buttonHTML;

  const filterButtons = buttons.querySelectorAll(".btn-item");
